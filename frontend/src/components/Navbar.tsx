...
export default function Navbar({ currentRoute, onRouteChange, onSelectTeam }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (diff <= 0) { setCountdownText(t("menu.countdown_started")); return; }
      const d = Math.floor(diff / 86400000);
      const h = Math.floor((diff % 86400000) / 3600000);
      const m = Math.floor((diff % 3600000) / 60000);
      const s = Math.floor((diff % 60000) / 1000);
      const isTr = i18n.language === "tr";
      setCountdownText(isTr ? `${d}G ${h}S ${m}D ${s}SN` : `${d}d ${h}h ${m}m ${s}s`);
    };
    tick();
  ...
  const navLinks = [
    ...
    { id: "elo",       label: t("menu.elo") },
    { id: "simulator", label: t("menu.simulator") },
    ...
    <header className={`sticky top-0 z-50 w-full flex flex-col transition-all duration-200 ${scrolled ? "shadow-md" : ""}`}>
            ...
            <div className="ticker-track">
              {[1, 2].map(i => (
                <span key={i} className="flex items-center gap-8 pr-16" style={{ color: "#8C8A84", fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.1em" }}>
                  <span style={{ color: "#F2F0E8" }}>{t("menu.countdown_prefix")}</span>
                  <span style={{ color: "#00FF87", fontVariantNumeric: "tabular-nums" }}>{countdownText}</span>
                  <span style={{ color: "rgba(255,255,255,0.2)" }}>//</span>
                  <span>{t("menu.ticker_location_title")}</span>
                  <span style={{ color: "rgba(255,255,255,0.2)" }}>//</span>
                  <span>{t("menu.ticker_details")}</span>
                  <span style={{ color: "rgba(255,255,255,0.2)" }}>//</span>
                </span>
              ))}
            </div>
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 px-4 md:px-6 h-14">
          ...
          <div className="hidden lg:flex items-center gap-1">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {dropdownOpen && (
                ...
                >
                  {[
                    { id: "teams", label: t("menu.all_teams") },
                    { id: "groups", label: t("menu.groups") },
                    { id: "matches", label: t("menu.matches") },
                  ].map(item => (
                    <a
                      key={item.id}
                      href={`#/${item.id}`}
          ...
          {/* Right Accessories (Search + Language Switcher) */}
              ...
              {showSearchResults && filteredTeams.length > 0 && (
                ...
                >
                  <div style={{ padding: "6px 12px", fontSize: "0.5rem", fontWeight: 900, letterSpacing: "0.18em", textTransform: "uppercase", color: "#8C8A84", borderBottom: "1px solid #EAE7DA" }}>
                    {t("menu.results", { count: filteredTeams.length })}
                  </div>
                  {filteredTeams.map(t => (
                    <button
                      key={t.teamId}
      ...
      {mobileMenuOpen && (
        ...
        >
          {[
            ...
            { id: "about",     label: t("menu.about") },
            { id: "elo",       label: t("menu.elo") },
            { id: "simulator", label: t("menu.simulator_cockpit") },
            { id: "contact",   label: t("menu.contact") },
          ].map(item => (
            <a
              key={item.id}
